...
const transporter = nodemailer.createTransport(sendgrid({
    auth:{api_key:key.SEND_GRID_API_KEY}
}))
...
router.post('/reset', (req,res)=>{
    try{
        crypto.randomBytes(32,async (err,buffer)=>{
            if(err){
                req.flash('error','Что-то пошло не так, пожалуйста повторите позже')
            ...
            if(candidate){
                ...
                candidate.resetTokenExp = Date.now() +  3600 * 1000 // 1 hour
...
router.post('/password',async (req,res)=>{
    try{
        ...
        if(user){
            ...
            req.flash('loginError','Время жизни токена истек')
            res.redirect('/auth/login')
        }
    }catch(e){
        console.log(e)
